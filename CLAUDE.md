...
**Current status: Stage 3.5 code done (one difficulty axis — `intensity` is gone; downgrade versions as their own rows; cycle days 1–3 held at tier `low`; body data re-read on every foreground with a confirmed re-plan. `SchemaV2`, 337 Core tests). Stage 4 code done (month calendar, day detail, rating summary). Stage 3, 3.5 and 4 are all open until their device checks in `DEV_PLAN.md` pass.** Stage 1 (day generation, catch-up, sampling, scoring, streak, ledger, today page, payout reveal, JSON/CSV exports, rating/comment log) is done. Stage 2: frequency scheduling, routine occurrences, routine load → random slots, routine completion (incl. late half pay), the overdue ladder + day-4 auto-skip, flexible Sunday settlement, backlog section, flexible do-ahead, a two-week `ScenarioTests`, simulator-only time travel on the debug page, ad-hoc replacement, and degraded_text (light version on low days, original still choosable). 250 Core tests. `doc/DEV_PLAN.md` is the task-level truth; read it before starting anything.

`tier` comes from `Energy.inputs` (HealthKit via `HealthService`) and is fixed when the day is generated, but body data is re-read on every foreground: when it disagrees, `Replan` offers to re-plan **only what is still open** — finished work is never touched. Cycle days 1–3 are capped at tier `low`, which is the only cycle rule there is. Auto-verify evidence is passed to `ensureToday` as `[dayKey: DayEvidence]`. Routine load is **not** an input — `ensureToday` schedules the day's routines and counts them itself.
...
- **One metric for one thing.** `intensity` was removed because difficulty already graded how hard a thing is; don't reintroduce a second axis for "how demanding". A routine's lighter version is a `RoutineTask` row with its own points and rule, not a text field.
