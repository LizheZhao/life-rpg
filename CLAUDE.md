...
**Current status: Stage 4 code done (month calendar, day detail, rating summary; 317 Core tests). Stage 3 is still open until its device checks in `DEV_PLAN.md` pass.** Stage 1 (day generation, catch-up, sampling, scoring, streak, ledger, today page, payout reveal, JSON/CSV exports, rating/comment log) is done. Stage 2: frequency scheduling, routine occurrences, routine load → random slots, routine completion (incl. late half pay), the overdue ladder + day-4 auto-skip, flexible Sunday settlement, backlog section, flexible do-ahead, a two-week `ScenarioTests`, simulator-only time travel on the debug page, ad-hoc replacement, and degraded_text (light version on low days, original still choosable). 250 Core tests. `doc/DEV_PLAN.md` is the task-level truth; read it before starting anything.

`tier` comes from `Energy.inputs` (HealthKit via `HealthService`), locked when the day is generated; auto-verify evidence is passed to `ensureToday` as `[dayKey: DayEvidence]`. Routine load is **not** an input — `ensureToday` schedules the day's routines and counts them itself.
